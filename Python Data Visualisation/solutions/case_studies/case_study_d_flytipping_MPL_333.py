...
# Fix the axis - the horizontal line goes wandering if we don't do this.
axes.axis([0, flytipping.shape[1], flytipping.shape[0],-1])   
...
header = plt.table(cellText=[["Number of fly-tipping incidents by waste type"]], cellLoc="left", loc="left",
                   bbox=[0.12, 0.97, 0.888, 0.12]) # x, y , length, height
...
# The alignment has been removed here as it was causing issues.
        cell.set_edgecolor("black")
        cell.set_linewidth(1)
        cell.PAD = 0.04
...
plt.figtext(x=0.03, y=0.03, s=caption, ha="left", fontweight="light", fontsize=10, fontname="sans-serif")
